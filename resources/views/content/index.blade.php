    ...
    <section id="new-content-wrapper">
        ...
        <div class="txta-wrapper">
            <form action="{{ route('contents.store') }}" method="POST" id="new-content" class="txta-wrapper">  
                ...
                @if ($setting->public_mode)
                    <label for="new-public" class="btn small-btn bg-main hover-effect small-public-btn new-public-btn">
                        <input type="checkbox" id="new-public" name="public" class="small-checkbox">public
                    </label>
                @endif
            </form>
            <div class="btn-wrapper flex">
                ...
                <button form="new-content" type="submit" class="btn small-btn hover-effect bg-main border-rb border-r-set">Save</button>
            </div>
        </div>
    </section>
    ...
    <section id="contents-section">
        @forelse ($contents as $content)
            <div id="content-wrapper">
                <div class="flex">
                    <span class="ml-small mr-small font-small font-{{ $setting->font_style_id }}">
                        {{ $content->created_at->isoFormat("dddd, MMMM D, YYYY h:mm A") }}
                    </span> 
                    @if ($setting->public_mode)
                ...
                <div class="txta-wrapper">
                    ...
                    <form id="content-{{ $content->id }}" action="{{ route('contents.update', $content->id) }}" method="POST">
                        ...
                        <trix-editor id="trix-editor" toolbar="hidden-toolbar-{{ $content->id }}" input="x-{{ $content->id }}"
                            class="editor-def set-h font-{{ $setting->font_style_id }}" contenteditable="false">
                        </trix-editor>         
                        <x-edit-remove-icon :isPublic="$setting->public_mode" />
                        <x-edit-icon />
                        @if ($setting->public_mode)
